...
// #include <msgs_veh/wfCarDataMsg.h>
#include <msgs_veh/wfRawDataMsg.h>
...
#include <arbe_msgs/wfAdasInput.h>
...
private:
  ...
  // 发布某个时间区间内的所有传感器消息(每个topic的msg全部发布)
  void publishIntervalMessages(const BagReader::IntervalMessages& interval_msgs,
                               const int& interval_idx);

  // /play_single_frame_X 服务回调:外部算法处理完一个区间后调用,确认OK并推进下一区间
  ...
  ros::Publisher pointcloud_sp_pub5_;

  ros::Publisher pointcloud_sgu_msgs1_;
  ros::Publisher pointcloud_sgu_msgs2_;
  ros::Publisher pointcloud_sgu_msgs3_;
  ros::Publisher pointcloud_sgu_msgs4_;

  ros::Publisher pointcloud_gt_msgs3_;
  ros::Publisher pointcloud_gt_msgs4_;
  ...
  ros::Publisher sgu_adas_input_msgs1_;
  ros::Publisher sgu_adas_input_msgs2_;
  ros::Publisher sgu_adas_input_msgs3_;
  ros::Publisher sgu_adas_input_msgs4_;



  // ros服务:外部算法确认处理OK后触发推进下一区间
