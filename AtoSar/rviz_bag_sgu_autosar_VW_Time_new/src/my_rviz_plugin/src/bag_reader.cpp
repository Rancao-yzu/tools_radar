...
int BagReader::getMainRadarSize() const
{
  ...
  {
  case 0:  return bPlaySPFlag_ ? pointcloud_sp_msgs0_.size() : pointcloud_msgs0_.size();
  case 1:  return bPlaySPFlag_ ? pointcloud_sp_msgs1_.size() : pointcloud_sgu_msgs1_.size();
  case 2:  return bPlaySPFlag_ ? pointcloud_sp_msgs2_.size() : pointcloud_sgu_msgs2_.size();
  case 3:  return bPlaySPFlag_ ? pointcloud_sp_msgs3_.size() : pointcloud_sgu_msgs3_.size();
  default: return bPlaySPFlag_ ? pointcloud_sp_msgs4_.size() : pointcloud_sgu_msgs4_.size();
  }
}
...
ros::Time BagReader::getMainRadarTime(int curIdx)
{
  ...
  switch (mainRadarIndex_)
  {
  case 0:  result = (bPlaySPFlag_ ? pointcloud_sp_msgs0_ : pointcloud_msgs0_)[curIdx].getTime(); break;
  case 1:  result = (bPlaySPFlag_ ? pointcloud_sp_msgs1_ : pointcloud_sgu_msgs1_)[curIdx].getTime(); break;
  case 2:  result = (bPlaySPFlag_ ? pointcloud_sp_msgs2_ : pointcloud_sgu_msgs2_)[curIdx].getTime(); break;
  case 3:  result = (bPlaySPFlag_ ? pointcloud_sp_msgs3_ : pointcloud_sgu_msgs3_)[curIdx].getTime(); break;
  default: result = (bPlaySPFlag_ ? pointcloud_sp_msgs4_ : pointcloud_sgu_msgs4_)[curIdx].getTime(); break;
  }
  return result;
}
...
void BagReader::collectIntervalMessages(const ros::Time& start, const ros::Time& end, IntervalMessages& out)
{
  ...
  collectFromRange(out[0],  start, end, bPlaySPFlag_ ? pointcloud_sp_msgs0_  : pointcloud_msgs0_);
  collectFromRange(out[1],  start, end, bPlaySPFlag_ ? pointcloud_sp_msgs1_  : pointcloud_sgu_msgs1_);
  collectFromRange(out[2],  start, end, bPlaySPFlag_ ? pointcloud_sp_msgs2_  : pointcloud_sgu_msgs2_);
  collectFromRange(out[3],  start, end, bPlaySPFlag_ ? pointcloud_sp_msgs3_  : pointcloud_sgu_msgs3_);
  collectFromRange(out[4],  start, end, bPlaySPFlag_ ? pointcloud_sp_msgs4_  : pointcloud_sgu_msgs4_);
  // idx5: warning_status
  ...
  // idx13-16: adas_input 1-4
  collectFromRange(out[13], start, end, sgu_adas_input_data1_);
  collectFromRange(out[14], start, end, sgu_adas_input_data2_);
  collectFromRange(out[15], start, end, sgu_adas_input_data3_);
  collectFromRange(out[16], start, end, sgu_adas_input_data4_);
  // idx17-18: gt sgu 3-4
  collectFromRange(out[17], start, end, pointcloud_gt_msgs3_);
  collectFromRange(out[18], start, end, pointcloud_gt_msgs4_);
  // idx19: IMU
  collectFromRange(out[19], start, end, IMU_msgs_);
}
