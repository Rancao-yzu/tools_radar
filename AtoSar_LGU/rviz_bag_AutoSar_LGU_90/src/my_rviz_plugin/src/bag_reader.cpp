...
BagReader::BagReader() : current_frame_(0), play_rate_(1.0), playing_(false), finishProcessFlag_(true), bPlaySPFlag_(false), mainRadarIndex_(1)  
{
...
int& frameCount3, int& frameCount4)
{
  ...
  object_sp_msgs0_.clear();
  object_sp_msgs1_.clear();
  object_sp_msgs2_.clear();
  object_sp_msgs3_.clear();
  object_sp_msgs4_.clear();
  ...
  std::vector<std::string> topics = {"/wf/corner_radar/lgu_data_0","/wf/corner_radar/lgu_data_1", "/wf/corner_radar/lgu_data_2", 
    ...
    "/wf/corner_radar/sgu_data_1", "/wf/corner_radar/sgu_data_2", "/wf/corner_radar/sgu_data_3", "/wf/corner_radar/sgu_data_4", 
  ...
  for (const auto& msg : view)
  {
    ...
    else if (msg.getTopic() == "/wf/corner_radar/sgu_data_0")
    {
      object_sp_msgs0_.push_back(msg);
    }
    else if (msg.getTopic() == "/wf/corner_radar/sgu_data_1")
    {
      object_sp_msgs1_.push_back(msg);
    }
    else if (msg.getTopic() == "/wf/corner_radar/sgu_data_2")
    {
      object_sp_msgs2_.push_back(msg);
    }
    else if (msg.getTopic() == "/wf/corner_radar/sgu_data_3")
    {
      object_sp_msgs3_.push_back(msg);
    }
    else if (msg.getTopic() == "/wf/corner_radar/sgu_data_4")
    {
      object_sp_msgs4_.push_back(msg);
    }
  ...
  for(int i=0;i<MAX_TOPIC_NUM;i++)
  {
    ...
    else
    {
      switch (i)
      {
        ...
        case 13:
          //sgu_0
          frame_msgs_.push_back(object_sp_msgs0_[msg_flags_[i]]);
          break;
        case 14:
          //sgu_1
          frame_msgs_.push_back(object_sp_msgs1_[msg_flags_[i]]);
          break;
        case 15:
          //sgu_2
          frame_msgs_.push_back(object_sp_msgs2_[msg_flags_[i]]);
          break;
        case 16:
          //sgu_3
          frame_msgs_.push_back(object_sp_msgs3_[msg_flags_[i]]);
          break;
        case 17:
          //sgu_4
          frame_msgs_.push_back(object_sp_msgs4_[msg_flags_[i]]);
          break;
        default:  
          break;
      }
...
void BagReader::jumpToFrame(int frame_number)
{
  ...
  if (frame_number >= 0 && frame_number < mainRadarSize)
  {
    ...
    {
      msg_flags_.resize(MAX_TOPIC_NUM, -1);
    ...
    msg_flags_[4] = findClosestPtFrame(selected_time,pointcloud_msgs4_);
      ...
      msg_flags_[13] = findClosestObjectSpFrame(selected_time,object_sp_msgs0_);
      msg_flags_[14] = findClosestObjectSpFrame(selected_time,object_sp_msgs1_);
      msg_flags_[15] = findClosestObjectSpFrame(selected_time,object_sp_msgs2_);
      msg_flags_[16] = findClosestObjectSpFrame(selected_time,object_sp_msgs3_);
      msg_flags_[17] = findClosestObjectSpFrame(selected_time,object_sp_msgs4_);
...
int BagReader::findClosestObjectSpFrame(const ros::Time& selected_time, std::vector<rosbag::MessageInstance>& object_sp_msgs)
{
    int closest_index = -1;
    double min_diff = std::numeric_limits<double>::max();

    auto& auto_msgs = object_sp_msgs;
