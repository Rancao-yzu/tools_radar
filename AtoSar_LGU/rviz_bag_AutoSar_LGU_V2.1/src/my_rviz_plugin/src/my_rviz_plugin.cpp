...
MyRvizPlugin::MyRvizPlugin(QWidget* parent)
  ...
  frame_count3(0), frame_count4(0),mainRadarIndex_(0)
{
  nh_ = ros::NodeHandle();
...
{ // 发布当前时间区间内所有传感器消息到ROS网络(区间内每个topic的所有msg全部发布)
...

  {
    ...
    auto pubSgu = [&](int idx, ros::Publisher& pub, const std::string& err)
    {
      ...
      {
        auto d = m.instantiate<arbe_msgs::wfAutosarData>();
        if (d) pub.publish(*d);
        else ROS_INFO("%s", err.c_str());
      }
    };
    pubSgu(0, pointcloud_pub0_, "pointcloud_data0 is null");
    pubSgu(1, pointcloud_pub1_, "pointcloud_data1 is null");
    pubSgu(2, pointcloud_pub2_, "pointcloud_data2 is null");
    pubSgu(3, pointcloud_pub3_, "pointcloud_data3 is null");
    pubSgu(4, pointcloud_pub4_, "pointcloud_data4 is null");
    ...
    if (!interval_msgs[3].empty()){
      auto d3 = interval_msgs[3][0].instantiate<arbe_msgs::wfAutosarData>();
      if (d3){
  ...
  //—————IMU—————//
  for (const auto& m : interval_msgs[13])
  {
...
void MyRvizPlugin::readBagFile()// 2025/9/17
{
  ...
  // 区间数:由主雷达首帧时间T0按15ms切片得到(切换主雷达后会重算)
