...
int& frameCount3, int& frameCount4)
{
  ...
  for (const auto& msg : view)
  {
    ...
    //update camera 6,7,8,9
    else if (msg.getTopic() == "/cv_camera_6/image_raw/compressed")
    {
      camera_msgs6_.push_back(msg);
    }
    else if (msg.getTopic() == "/cv_camera_7/image_raw/compressed")
    {
      camera_msgs7_.push_back(msg);
    }
    else if (msg.getTopic() == "/cv_camera_8/image_raw/compressed")
    {
      camera_msgs8_.push_back(msg);
    }
    else if (msg.getTopic() == "/cv_camera_9/image_raw/compressed")
    {
      camera_msgs9_.push_back(msg);
    }
  ...
  for(int i=0;i<MAX_TOPIC_NUM;i++)
  {
    ...
    else
    {
      switch (i)
      {
        ...
        case 14:
          //camera 6
          frame_msgs_.push_back(camera_msgs6_[msg_flags_[i]]);
          break;
        case 15:
          frame_msgs_.push_back(camera_msgs7_[msg_flags_[i]]);
          break;
        case 16:
          frame_msgs_.push_back(camera_msgs8_[msg_flags_[i]]);
          break;
        case 17:
          frame_msgs_.push_back(camera_msgs9_[msg_flags_[i]]);
          break;
        default:  
          break;
      }
...
void BagReader::jumpToFrame(int frame_number)
{
  ...
  if (frame_number >= 0 && frame_number < mainRadarSize)
  {
    ...
    if (message_callback_)
    {
      ...
      msg_flags_[14]= findClosestCameraFrame(selected_time,camera_msgs6_);
      msg_flags_[15] = findClosestCameraFrame(selected_time,camera_msgs7_);
      msg_flags_[16] = findClosestCameraFrame(selected_time,camera_msgs8_);
      msg_flags_[17] = findClosestCameraFrame(selected_time,camera_msgs9_);
