...
class TimeStampMatcher:
    ...
    def process_radar_topic(self, bag_file, bag_name, topic, radar_name, output_dir):
        ...
        try:
            ...
            for msg_topic, msg, t in bag.read_messages(topics=[topic]):
                ...
                if frame_header_bytes != 0x5AA5:  # 固定帧头
                    continue
    ...
    def parse_single_target(self, obj_data):
        ...
        try:
            ...
            # 航向角：从度转换为弧度
            yaw_ang_deg = struct.unpack_from('<h', obj_data, offset)[0] / 100.0
            result['yaw_ang'] = yaw_ang_deg * (3.1416 / 180.0)
            offset += 2
        ...
        for target in radar_targets:
            ...
            if time_diff < 0.1:  # 100ms阈值
                matched_record = {
                    ...
                    'ID': target['obj_id'],
                    'RxReal': target['dist_x'],
                    'RyReal': target['dist_y'],
                    'Vx': target['velAbsX'],
                    'Vy': target['velAbsY'],
