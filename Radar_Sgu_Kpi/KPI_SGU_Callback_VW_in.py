...
class TimeStampMatcher:
    ...
    def extract_imu_data(self, bag_file, bag_name):
        ...
        try:
            ...
            for topic, msg, t in bag.read_messages(topics=[self.imu_topic]):
                ...
                if t.to_sec() > 1790815698:  # 2026年10月1日的时间戳 1790815698
                    bag.close()
                    raise Exception(f"提取IMU数据时ERROR - TIME EXPIRED -{t.to_sec()} > 2026年10月")
            ...
            for msg_topic, msg, t in bag.read_messages(topics=[topic]):
                ...
                for obj_idx, obj in enumerate(msg.ObjectsBuffer):
                    ...
                    target_info = {
                        ...
                        'velAbsX': obj.Vx if hasattr(obj, 'Vx') else 0.0,
                        'velAbsY': obj.Vy if hasattr(obj, 'Vy') else 0.0,
                        'accelAbsX': obj.accelX if hasattr(obj, 'accelX') else 0.0,
                        'accelAbsY': obj.accelY if hasattr(obj, 'accelY') else 0.0,
                        'yaw_ang': (obj.Ang if hasattr(obj, 'Ang') else 0.0)* (3.1416 / 180.0) ,
                        ...
                        'obj_id': obj.ID if hasattr(obj, 'ID') else 0,
    ...
    def match_and_save_radar_data(self, bag_name, radar_name, radar_targets, output_dir):
        ...
        for target in radar_targets:
            ...
            # 在IMU时间戳中寻找插值区间 [idx-1, idx]，使 target 时间戳落在区间内
            idx = np.searchsorted(imu_timestamps, target['timestamp'])
            if idx == 0 or idx >= len(imu_timestamps):
                # 目标时间戳超出IMU数据范围，无法插值
                skipped_count += 1
                continue
            left_imu = self.imu_data[idx - 1]
            right_imu = self.imu_data[idx]
            t_left = imu_timestamps[idx - 1]
            t_right = imu_timestamps[idx]
            
            # 插值区间时间跨度过大时，线性插值不可靠，跳过
            gap = t_right - t_left
            if gap > 0.2:  # 200ms阈值
                skipped_count += 1
                continue
            
            # 插值比例 (0~1)，防止重复时间戳导致除零
            ratio = 0.0 if gap == 0 else (target['timestamp'] - t_left) / gap
            
            # 对IMU字段做线性插值
            def interp_imu(field):
                v_left = left_imu.get(field, 0)
                v_right = right_imu.get(field, 0)
                return v_left + (v_right - v_left) * ratio
            
            # 插值区间内时间上最近的IMU点 (原始取点方式)
            nearest_imu = left_imu if ratio <= 0.5 else right_imu
            
            # 逐字段选择: 插值值与最近点值, 谁与雷达测量值误差更小就用谁
            def choose_gt_value(field, radar_value):
                v_nearest = nearest_imu.get(field, 0)
                v_interp = interp_imu(field)
                if abs(v_interp - radar_value) < abs(v_nearest - radar_value):
                    return v_interp
                return v_nearest
            ...
            matched_record = {
                ...
                # IMU数据 (逐字段对比插值与最近点, 取与雷达测量更接近者)
                'GT_LngDist': choose_gt_value('GT_LngDist', target['RxReal']),
                'GT_LatDist': choose_gt_value('GT_LatDist', target['RyReal']),
                'GT_LngSpeed': choose_gt_value('GT_LngSpeed', target['velAbsX']),
                'GT_LatSpeed': choose_gt_value('GT_LatSpeed', target['velAbsY']),
                'AcceX_GT': choose_gt_value('AcceX_GT', target['accelAbsX']),
                'AcceY_GT': choose_gt_value('AcceY_GT', target['accelAbsY']),
                'YawAng_GT': choose_gt_value('YawAng_GT', target['yaw_ang']),
                
                # 元数据 (插值后时间戳精确对齐)
                'time_diff': 0.0,
            }
            
            # 记录插值区间左端点IMU索引，用于后续按区间筛选
            matched_records.append((idx - 1, matched_record))
        
        # 每个IMU插值区间只保留与其最相近的3个目标 (按 |RxReal - GT_LngDist| 从小到大排序)
        imu_target_map = {}
        ...
        if final_records:
            self.log(f"    匹配成功: {len(final_records)} 个目标 (每个IMU插值区间保留最多3个, 跳过 {skipped_count} 个超出IMU时间范围的目标)")
            # 保存CSV文件
            return self.save_radar_csv(bag_name, radar_name, final_records, output_dir)
        else:
            self.log(f"    没有匹配到目标 (跳过 {skipped_count} 个超出IMU时间范围的目标)")
            return None
    ...
    def save_radar_csv(self, bag_name, radar_name, matched_records, output_dir):
        ...
        csv_filename = f"{bag_name_csv}_{radar_name}_into.csv"
...
class MatcherGUI:
    ...
    def setup_ui(self):
        ...
        # 本次修改内容说明
        self.change_info_label = ttk.Label(
            main_frame,
            text="本次修改内容:\n"
                 " 1. IMU数据插值对齐 \n"
                 " 2. 逐字段更优值选择: 插值值与最近点值, 谁与雷达测量误差小就用谁\n"
                 " 3. CSV输出新增 time_diff 列\n"
                 " 4. 新增IMU时间偏移 x ms (-120~120, 默认0, 正数=IMU向后延时)",
            justify=tk.LEFT, foreground='#cc0000')
        self.change_info_label.grid(row=row, column=0, columnspan=2, sticky=tk.W, pady=5)
        row += 1
