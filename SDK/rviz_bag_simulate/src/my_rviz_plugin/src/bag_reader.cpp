  ...
  int& frameSPCount2,int& frameCount3, int& frameSPCount3,int& frameCount4, int& frameSPCount4)
{
  ...
  simulate_msgs0_.clear();
  simulate_msgs1_.clear();
  simulate_msgs2_.clear();
  simulate_msgs3_.clear();
  simulate_msgs4_.clear();
  empty_msgs_.clear();
  ...
  std::vector<std::string> topics = {"/wf/corner_radar/parsed/float_data_0","/wf/corner_radar/parsed/float_data_1", "/wf/corner_radar/parsed/float_data_2", 
    ...
    "/wf/corner_radar/simulate_data_0","/wf/corner_radar/simulate_data_1","/wf/corner_radar/simulate_data_2",
    "/wf/corner_radar/simulate_data_3","/wf/corner_radar/simulate_data_4",
  ...
  for (const auto& msg : view)
  {
    ...
    else if (msg.getTopic() == "/wf/corner_radar/parsed/float_data_0")
    {
      pointcloud_msgs0_.push_back(msg);
    }
    else if (msg.getTopic() == "/wf/corner_radar/parsed/float_data_1")
    {
      pointcloud_msgs1_.push_back(msg);
    }
    else if (msg.getTopic() == "/wf/corner_radar/parsed/float_data_2")
    {
      pointcloud_msgs2_.push_back(msg);
    }
    else if (msg.getTopic() == "/wf/corner_radar/parsed/float_data_3")
    {
      pointcloud_msgs3_.push_back(msg);
    }
    else if (msg.getTopic() == "/wf/corner_radar/parsed/float_data_4")
    {
      pointcloud_msgs4_.push_back(msg);
    }
    else if(msg.getTopic() == "/wf/frame_rd_data/ti/radar_0")
    {
      pointcloud_sp_msgs0_.push_back(msg);
    }
    else if(msg.getTopic() == "/wf/frame_rd_data/ti/radar_1")
    {
      pointcloud_sp_msgs1_.push_back(msg);
    }
    else if(msg.getTopic() == "/wf/frame_rd_data/ti/radar_2")
    {
      pointcloud_sp_msgs2_.push_back(msg);
    }
    else if(msg.getTopic() == "/wf/frame_rd_data/ti/radar_3")
    {
      pointcloud_sp_msgs3_.push_back(msg);
    }
    else if(msg.getTopic() == "/wf/frame_rd_data/ti/radar_4")
    {
    ...
    else if(msg.getTopic() == "/wf/corner_radar/simulate_data_0")
    {
      simulate_msgs0_.push_back(msg);
    }
    else if(msg.getTopic() == "/wf/corner_radar/simulate_data_1")
    {
      simulate_msgs1_.push_back(msg);
    }
    else if(msg.getTopic() == "/wf/corner_radar/simulate_data_2")
    {
      simulate_msgs2_.push_back(msg);
    }
    else if(msg.getTopic() == "/wf/corner_radar/simulate_data_3")
    {
      simulate_msgs3_.push_back(msg);
    }
    else if(msg.getTopic() == "/wf/corner_radar/simulate_data_4")
    {
      simulate_msgs4_.push_back(msg);
    }
  ...
  for(int i=0;i<MAX_TOPIC_NUM;i++)
  {
    ...
    else
    {
      switch (i)
      {
        ...
        case 13:
          //simulate 0
          frame_msgs_.push_back(simulate_msgs0_[msg_flags_[i]]);
          break;
        case 14:
          //simulate 1
          frame_msgs_.push_back(simulate_msgs1_[msg_flags_[i]]);
          break;
        case 15:
          //simulate 2
          frame_msgs_.push_back(simulate_msgs2_[msg_flags_[i]]);
          break;
        case 16:
          //simulate 3
          frame_msgs_.push_back(simulate_msgs3_[msg_flags_[i]]);
          break;
        case 17:
          //simulate 4
          frame_msgs_.push_back(simulate_msgs4_[msg_flags_[i]]);
          break;
        default:  
          break;
      }
...
void BagReader::jumpToFrame(int frame_number)
{
  ...
  if (frame_number >= 0 && frame_number < mainRadarSize)
  {
    ...
    if (message_callback_)
    {
      ...
      // if (current_frame_ == 0) {
      //   // 将所有消息标志设置为-1，表示使用空消息
      //   // for (int j = 0; j < msg_flags_.size(); j++) 
      //   //     msg_flags_[j] = -1;
        
      //   packetCallbackMsg();
      //   message_callback_(frame_msgs_, current_frame_, msg_flags_);
      //   ROS_INFO("Playing empty frame 0");
      //   return;
      // }
      ...
      msg_flags_[13] = findClosestSimulateFrame(selected_time,simulate_msgs0_);
      msg_flags_[14] = findClosestSimulateFrame(selected_time,simulate_msgs1_);
      msg_flags_[15] = findClosestSimulateFrame(selected_time,simulate_msgs2_);
      msg_flags_[16] = findClosestSimulateFrame(selected_time,simulate_msgs3_);
      msg_flags_[17] = findClosestSimulateFrame(selected_time,simulate_msgs4_);
...
void BagReader::setPlayFinishedCallback(PlayFinishedCallback callback)
{
  play_finished_callback_ = callback;
}
...
int BagReader::findClosestPtFrame(const ros::Time& selected_time, std::vector<rosbag::MessageInstance>& pcl_msgs, std::vector<rosbag::MessageInstance>& pcl_sp_msgs)
{
    ...
    else
    {
      for (size_t i = 0; i < pcl_sp_msgs.size(); ++i) {
        ros::Time pt_time = pcl_sp_msgs[i].getTime();
        double time_diff = std::abs((selected_time - pt_time).toSec());
...
int BagReader::findClosestSimulateFrame(const ros::Time& selected_time, std::vector<rosbag::MessageInstance>& simulate_msgs)
{
    int closest_index = -1;
    double min_diff = std::numeric_limits<double>::max();

    for (size_t i = 0; i < simulate_msgs.size(); ++i) {
        ros::Time msg_time = simulate_msgs[i].getTime();
...
void BagReader::playLoop() 
{
  ...
  for (size_t i = 0; i < mainRadarSize; ++i) 
  {
    ...
    message_callback_(frame_msgs_,current_frame_,msg_flags_);
    //ROS_INFO("Playing frame %d", current_frame_);
    std::this_thread::sleep_for(std::chrono::milliseconds((int)(5 / play_rate_)));//ms/1s
  }

  if (play_finished_callback_)
    play_finished_callback_();
}
